...
/**
 * True when an expense is still active FOR A SPECIFIC USER — their own share
 * is not yet PAID or PENDING_VERIFICATION. The dashboard hides expenses the
 * user has already settled (including awaiting approval), even if other
 * members still owe money.
 */
export function isExpenseActiveForUser(expense, userId) {
  ...
  // Hide if the user has already paid or is awaiting verification — they've
  // done their part.
