...
/**
 * Find an existing user by email, or create a new one for a Google sign-in.
 * Google users don't have a passwordHash (null). If the email already exists
 * (registered via email/password), we just return the existing user — Google
 * sign-in becomes an alternate login method for the same account.
 */
export async function findOrCreateGoogleUser({ email, name, image }) {
...
export async function getUser(uid) {
  ...
  // Parse category limits from JSON string to object (dollars)
  let categoryLimits = null;
  if (u.categoryLimits) {
    try {
      const raw = JSON.parse(u.categoryLimits);
      categoryLimits = {};
      for (const [cat, cents] of Object.entries(raw)) {
        categoryLimits[cat] = cents / 100;
      }
    } catch { /* ignore malformed JSON */ }
  }

  // Fetch room info if user is in a room
  let room = null;
  if (u.roomId) {
    const r = await prisma.room.findUnique({ where: { id: u.roomId } });
    if (r) {
      room = { id: r.id, name: r.name, code: r.code, ownerId: r.ownerId };
    }
  }
  ...
  return {
    ...
    dailyLimit: u.dailyLimitCents != null ? fromCents(u.dailyLimitCents) : null,
    categoryLimits,
    roomId: u.roomId,
    room,
  };
...
/** Add funds to the user's balance (incremental). Returns new balance in dollars. */
export async function addFunds(uid, amount) {
  const user = await prisma.user.findUnique({ where: { id: uid }, select: { balance: true } });
  const currentCents = user?.balance ?? 0;
  const addCents = toCents(amount);
  const newCents = currentCents + addCents;

  await prisma.user.update({
    where: { id: uid },
    data: { balance: newCents },
  });

  return fromCents(newCents);
}
 ...
 * exactly the list passed in, which may or may not include the payer. When
 * the payer is in the split their own share is created PAID (they paid up
 * front); every other member starts PENDING. If the payer isn't in the split,
 * the full amount is split among the selected members only and the payer is
 * owed the total. Shares are stored in cents.
 *
 * Wallet effect: the payer's personal balance drops by the full amount they
 * paid up front.
 */
export async function createExpense({ roomId, title, description, category, amount, paidBy, createdBy, splitBetween }) {
 ...
 * shaped as a uid → share map, ready for the summary helpers. When `uid` is
 * passed, only expenses involving that user — ones they paid for OR were
 * split with — are returned; unrelated room expenses stay hidden.
 */
export async function getExpenses(roomId, uid) {
  const rows = await prisma.expense.findMany({
    where: {
      roomId,
      ...(uid
        ? {
            OR: [{ paidBy: uid }, { shares: { some: { userId: uid } } }],
          }
        : {}),
    },
